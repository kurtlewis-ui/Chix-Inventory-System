        ...
        <div className="border-t border-nav-border px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              {user?.branch?.name && (
                <p className="text-[11px] text-nav-text truncate">{user.branch.name}</p>
              )}
            </div>
...
function DraftBag() {
  ...
  return (
    <>
      ...
      <button
        onClick={() => { setOpen((o) => !o); }}
        className={`fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full shadow-lg shadow-black/30 hover:opacity-90 transition ${contentTheme === 'light' ? 'bg-[#0f1b34] text-white' : 'bg-white text-black'}`}
        title="Draft order"
